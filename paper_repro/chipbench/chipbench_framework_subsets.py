...
SRC_BASE = '/ssd2/yichen/upstream/ChipBench/Verilog Gen'
...
def main():
    ...
    subsets = ['dataset_not_self_contain', 'dataset_cpu_ip']
    # qualify each problem with its subset so the per-problem (spec,ref,tb) load uses the right dir
    probs_by_subset = {}
    for ss in subsets:
        for p in glob.glob(f'{SRC_BASE}/{ss}/Prob*_prompt.txt'):
            name = os.path.basename(p).replace('_prompt.txt', '')
            probs_by_subset[f'{ss}::{name}'] = ss
    probs = sorted(probs_by_subset.keys())
    ...
    pre = {}
    for p in probs:
        ss = probs_by_subset[p]
        name = p.split('::', 1)[1]
        d = f'{SRC_BASE}/{ss}'
        spec = open(f'{d}/{name}_prompt.txt').read()
        ref = open(f'{d}/{name}_ref.sv').read()
        # no strict TB for not_self_contain / cpu_ip; use original
        tb = open(f'{d}/{name}_test.sv').read()
        pre[p] = (spec, ref, tb)
